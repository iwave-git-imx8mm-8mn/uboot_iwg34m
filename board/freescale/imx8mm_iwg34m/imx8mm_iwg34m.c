...
static iomux_v3_cfg_t const board_cfg[] = {
      ...
      IMX8MM_PAD_SAI2_MCLK_GPIO4_IO27 | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
...
static printboard_info(void)
{
     ...
     printf ("\tSOM Version               : iW-PRFYZ-AP-01-R%x.%x\n",pcb_rev,bom_rev);
...
/*static void carrier_rst(void)
{
	gpio_request(IMX_GPIO_NR(5, 2), "Carrier Rst");
...
void iwg34m_fdt_update(void *fdt)
{
	int value=0;
	int reg[1];
